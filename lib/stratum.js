...
setInterval(() => { upstream.refreshWork() }, config.stratumServer.blockRefreshInterval)
...
function handleMinerData (method, params, socket, portData, difficulty, sendReply, sendReplyNH, pushMessage, makeJob, makeJobNH) {
  ...
  methods.eth_getWork = () => {
    const job = makeJob(socket.extraNonce)
    deasync.loopWhile(() => { return !job })
    sendReply(null, job)
  }
