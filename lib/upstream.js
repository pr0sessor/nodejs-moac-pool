 ...
 **/
  ...
  getBlockNumber (cb) {
    this.rpc('mc_getBlockByNumber', ['latest', false], (err, res) => {
  ...
  submitWork (nonce, powHash, mixHash, height, miner) {
    this.rpc('mc_submitWork', [nonce, powHash, mixHash], (err, result) => {
      if (err) return logger('error', 'upstream', 'Failed to submit work')
      if (result) {
        this.rpc('mc_getBlock', [height], (err2, block) => {
